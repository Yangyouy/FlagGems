...
"""
Hygon specialization for top_k_per_row_decode.

The default implementation uses tl.histogram with a `mask` parameter which is
not available on Hygon's Triton. This specialization replaces masked histogram
calls with: tl.where(mask, bucket, 0) + tl.histogram + subtract invalid count
from bin 0.
"""
...
@triton.jit
def _masked_histogram(bucket, num_bins: tl.constexpr, mask):
    """Emulate tl.histogram(bucket, num_bins, mask=mask) without mask param.

    Strategy: set masked positions to 0, compute histogram, subtract the
    number of invalid elements from bin 0.
    """
    masked_bucket = tl.where(mask, bucket, 0)
    counts = tl.histogram(masked_bucket, num_bins)
    n_invalid = tl.sum((~mask).to(tl.int32))
    bins = tl.arange(0, num_bins)
    counts = counts - tl.where(bins == 0, n_invalid, 0)
    return counts
...
def _topk_single_block(
    ...
    counts_0 = _masked_histogram(bucket_0, 256, valid)
    ...
    counts_1 = _masked_histogram(bucket_1, 256, match_0)
    ...
    counts_2 = _masked_histogram(bucket_2, 256, match_1)
    ...
    counts_3 = _masked_histogram(bucket_3, 256, match_2)
...
def _topk_medium_block(
    ...
    bucket_0 = (sortable >> 24) & 0xFF
    local_hist_0 = _masked_histogram(bucket_0, 256, valid)
    ...
    bucket_1 = (sortable >> 16) & 0xFF
    local_hist_1 = _masked_histogram(bucket_1, 256, match)
    ...
    bucket_2 = (sortable >> 8) & 0xFF
    local_hist_2 = _masked_histogram(bucket_2, 256, match)
    ...
    bucket_3 = sortable & 0xFF
    local_hist_3 = _masked_histogram(bucket_3, 256, match)
...
def _topk_multi_block(
    ...
    local_hist = _masked_histogram(bucket, 256, valid)
    ...
    if pid == 0:
        ...
        counts_1 = _masked_histogram(b_byte_1, 256, b_valid)
        total_1 = tl.sum(counts_1)
        ps_1 = tl.cumsum(counts_1, axis=0)
        ss_1 = total_1 - ps_1 + counts_1
        pivot_1 = tl.max(tl.where(ss_1 >= remaining_k, bins, -1))
        ...
        counts_2 = _masked_histogram(b_bucket_2, 256, b_match_2)
        total_2 = tl.sum(counts_2)
        ps_2 = tl.cumsum(counts_2, axis=0)
        ss_2 = total_2 - ps_2 + counts_2
        pivot_2 = tl.max(tl.where(ss_2 >= remaining_k, bins, -1))
        ...
        counts_3 = _masked_histogram(b_bucket_3, 256, b_match_3)
        total_3 = tl.sum(counts_3)
        ps_3 = tl.cumsum(counts_3, axis=0)
        ss_3 = total_3 - ps_3 + counts_3
        pivot_3 = tl.max(tl.where(ss_3 >= remaining_k, bins, -1))
...
def top_k_per_row_decode(
    ...
    """Top-K per row for decode phase (Hygon specialization).

    Same interface as the default implementation but uses _masked_histogram
    to work around the missing mask parameter in tl.histogram on Hygon.
    """
    logger.debug("GEMS TOP_K_PER_ROW_DECODE (Hygon)")
